...
use serum_dex_0_4_0::instruction::SelfTradeBehavior;
...
pub fn new_order_v3<'info>(
    ...
    let ix = serum_dex_0_4_0::instruction::new_order(
...
pub fn cancel_order_v2<'info>(
    ...
    let ix = serum_dex_0_4_0::instruction::cancel_order(
        &ID,
        ctx.accounts.market.key,
        ctx.accounts.market_bids.key,
        ctx.accounts.market_asks.key,
...
pub fn settle_funds<'info>(
    ...
    let referral = ctx.remaining_accounts.get(0);
    let ix = serum_dex_0_4_0::instruction::settle_funds(
        &ID,
        ctx.accounts.market.key,
        ctx.accounts.token_program.key,
        ctx.accounts.open_orders.key,
...
pub fn init_open_orders<'info>(
    ...
    let ix = serum_dex_0_4_0::instruction::init_open_orders(
        &ID,
        ctx.accounts.open_orders.key,
...
pub fn close_open_orders<'info>(
    ...
    let ix = serum_dex_0_4_0::instruction::close_open_orders(
        &ID,
        ctx.accounts.open_orders.key,
        ctx.accounts.authority.key,
...
pub fn sweep_fees<'info>(ctx: CpiContext<'_, '_, '_, 'info, SweepFees<'info>>) -> ProgramResult {
    let ix = serum_dex_0_4_0::instruction::sweep_fees(
        &ID,
        ctx.accounts.market.key,
        ctx.accounts.pc_vault.key,
...
pub fn initialize_market<'info>(
    ...
    let ix = serum_dex_0_4_0::instruction::initialize_market(
        ctx.accounts.market.key,
        &ID,

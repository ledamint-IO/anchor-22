...
fn generate_constraint_seeds(f: &Field, c: &ConstraintSeedsGroup) -> proc_macro2::TokenStream {
    ...
    if c.is_init && c.bump.is_some() {
        ...
        quote! {
            let (__program_signer, __bump) = anchor_lang::safecoin_program::pubkey::Pubkey::find_program_address(
                &[#s],
                program_id,
            );
...
pub fn generate_init(
    ...
    match kind {
        InitKind::Token { owner, mint } => {
            ...
            quote! {
                let #field: #ty_decl = {
                    if !#if_needed || #field.to_account_info().owner == &anchor_lang::safecoin_program::system_program::ID {
